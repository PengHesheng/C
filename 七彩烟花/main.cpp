...
void Lode();  //加载烟花图片
void Shoot();  //发射烟花
void Chose(DWOROD&);  //筛选烟花
void Style(DWORD&);  //发射模式
...
void main()
{
    ...
    micSendString("play bk repeat",0,0,0);
    welcome();
    DWORD t1 = timeGetTime();  //筛选烟花计时
    DWORD st1 = timeGetTime();  //播放烟花样计时
    DWORD*pMem = GetlmageBuffer();  //获取窗口显存指针
    ...
    for (int clr = 0; clr < 1000; clr++)
    {
        for (int j = 0; j < 2; j++)
        {
            ...
            int py2 = rand() % 800;
            if (py1 < 799)  //防止越界
            pMen[py1 * 1200 + px1] = pMem[py1 * 1200 + px1 + 1] = BLACK;  //显示前面的所有绘图操作
        }
    }
}
...
void welcome()
{
    ...
    for (int i = 0; i < 50; i++)
    {
        int x = 600 + int(800 * sim(PI * 2 * i / 60));
        int y = 200 + int(800 * cos(PI * 2 * i / 60));
        cleardevice();
        settextstyle(i,0,"楷体");
        outtextxy(x,y,"烟花");
        outtextxy(x,y - 50,"制作人：PHE");
        outtextxy(x - 130,y + 100,"I LOVE YOU!");
        Sleep(25);
    }
    getchar();
    cleardevice();
    ...
    outtextxy(400,300,"又他妈的换来了多少失望？");
    outtextxy(400,350,"烟花再美又能怎样？");
    outtextxy(400,400,"女孩自己要懂得自己挣钱");
    outtextcy(400,450,"会开车会打扮");
...
void Init(int i)
{
    //分别为：烟花中心到图片边缘的最远距离、左上角的距离（x、y）两个分量
...
//在一定范围内筛选可发射的烟花，并初始化发射参数，输出烟花弹道屏幕，播放声音
void Chose(DWORD&t1)
{
    ...
    {
        int n = rand() % 20;
        if (n < 13 && Je[n].shoot == false && Fire[n].show == false)
        {
            //重置烟花弹，预备发射
            Jet[n].x = rand() % 1200;
            ...
            //播放每个花弹的声音
            char ci[50],c2[30],c3[30];
            sprintf(c1,"open./fire/shoot.mp3 aliass%d"n);
            sprintf("c2,*play s%d",n);
            sprintf("c3,*close %d",n);
...
void Shoot()
{
    for (int i = 0; i < 13; i++)
    {
        ...
        {
            //烟花弹的上升
            putomage(Jet[i].x,Jet[i].y, &Jet[i].img[Jet[i].n],SRCINVERT);
            if (Jet[i].y > Jet[i].hy)
            {
                Jet[i].n++;
                Jet[i].y -= 5;
            ...
            if (Jet[i].y <= Jet[i].hy)
            {
                ...
                sprintf(c1,"open./fire/bomb.wavalias n%d",i);
                sprintf(c2,"play n%d",i);
                sprintf(c3,"close s%d",i);
                mciSendString(c3,0,0,0);
                mciSendString(c1,0,0,0);
                mciSendString(c2,0,0,0);
                putimage(Jet[i].x,Jet[i].y, &Jet[i].img[Jet[i].n],SRCINVER);  //擦掉烟花弹
    ...
    for (int i = 0; i < NUM; i++)
    {
        ...
        if (Fire[i].draw)
        {
            for (double a = 0; a <= 6.28; a += 0.01)
            {
                ...
                int y1 = (int)(Fire[i].cen_y - Fire[i].t * sin(a));
                if (x1 > 0 && x1 < Fire[i].width && y1 > 0 && y1 < Fire[i].height)  //只输出图片内的像素点
                {
                    ...
                    //较暗的像素点不输出、防止越界
                        if (r > 0x20 && g > 0x20 && b > 0x20 && xx >0 && x < 1200 && yy > 0 && yy < 800)
                        pMem[yy * 1200 +xx] = BGR(Fire[i].xy[x1][y1]);  //显示操作绘制烟花
                }
            }
            Fire[i].draw = false;
        }
